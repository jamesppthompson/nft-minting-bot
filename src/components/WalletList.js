...
export default function WalletList(props) {
  ...
  const delWallet = (wallet) => {
    const restWallet = wallets.filter(
    ...
    async function fetchWallets() {
      ...
      if (oldWallets === null || oldWallets === []) {
        ...
        for (let i = 0; i < oldWallets.length; i++) {
          ...
          wall = {
            walletName: oldWallets[i].walletName,
            address: oldWallets[i].address,
            balance,
            key: oldWallets[i].key,
          };
          temp = [...temp, wall];
        }
      ...
      {wallets !== null ? (
        wallets.map((wallet, id) => (
          <WalletCard
            ...
            wallet={wallet}
            taskForm={taskForm}
            setSelectWallet={(checked, wallet) =>
              setSelWallets(checked, wallet)
            }
            delWallet={(wallet) => delWallet(wallet)}
          />
        ))
      ) : (
